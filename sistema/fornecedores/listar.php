<?php
define("PAGINA", "LISTAR FORNECEDORES");
define("CSS", '
...
define("JS", '
  ...
  <script type="text/javascript" src="'.URL_SISTEMA.'/sistema/fornecedores/listar.js"></script>
  <!-- END PAGE VENDOR JS-->');
    ...
    <div class="content-wrapper">
      <div class="content-header row">
        <div class="content-header-left col-md-6 col-12 mb-2">
          <h3 class="content-header-title">FORNECEDORES</h3>
          <div class="row breadcrumbs-top">
            <div class="breadcrumb-wrapper col-12">
              <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?=URL_SISTEMA?>">Dashboard</a>
                </li>
                <li class="breadcrumb-item active">Fornecedores
                </li>
              </ol>
            </div>
          </div>
          ...
          <div class="media width-500 float-right">
            <div class="media-body media-right text-right">
			  ...
			  <div class="btn-group" role="group" aria-label="First Group">
				  ...
				  <a href="<?=URL_SISTEMA?>/fornecedores/novo" class="btn btn-icon btn-outline-primary"><i class="la la-plus"></i> NOVO FORNECEDOR</a>
				</div>
                  ...
                  <div class="card-body card-dashboard">
					<table class="table table-striped table-bordered sourced-data" id="listar">
					  ...
					  <tbody>	
						...
						?>
                        <tr id="Iten-<?=$ITEM->cod_fornecedor?>">
                          <td><input type="checkbox" class="largerCheckbox" id="itens" name="itens[]" value="<?=$ITEM->cod_fornecedor?>" /></td>
                          <td><?=($ITEM->cod) ? $ITEM->cod : "N/A";?></td>
                          <td><?=($ITEM->nome_fantasia) ? $ITEM->nome_fantasia : "NÃO INFORMADO";?></td>
                          <td><?=($ITEM->cpf) ? $ITEM->cpf : "N/A";?></td>
                          <td><?=($ITEM->telefone) ? $ITEM->telefone : "N/A";?></td>
                          <td><?=($ITEM->cidade) ? $ITEM->cidade."/".$ITEM->estado : "N/A";?></td>
                          <td><?=strftime("%d/%m/%Y",strtotime($ITEM->data_cadastro))?></td>
                          <td>
							<button type="button" onclick="remover(<?=$ITEM->cod_fornecedor?>)" class="btn btn-outline-danger mr-1 float-right ladda-button" data-style="zoom-in">
								<i class="la la-trash"></i>
							</button>
							<a href="<?=URL_SISTEMA?>/fornecedores/editar&id=<?=$ITEM->cod_fornecedor?>&key=true" class="btn btn-outline-info mr-1 float-right ladda-button" data-style="zoom-in">
