...
var financeiro = {
	receitas: function(form) {
		...
		$.ajax({
			type: "POST",
			url: URL_SISTEMA+"/financeiro/receitas/extrato",
			data: dados,
	...
	cancelar: function(ID){ 
		swal({
		  title: "Aviso",
		  text: "Você tem certeza que deseja cancelar essa receita?",
		  type: "warning",
						...
						if (inputValue === "") {
							...
							$.post(URL_SISTEMA+"/api/financeiro", {'acao': 'cancelarDespesa', 'cod':ID, 'senha': inputValue}, function(result){
								...
								if(retorno.resposta == 1) {
									...
									swal("CANCELADO", "Receita cancelada com sucesso!", "success");
	...
	liquidar: function(ID){ 
		swal({
		  title: "Aviso",
		  text: "Você tem certeza que deseja liquidar essa receita?",
		  type: "warning",
		  showCancelButton: true,
		  confirmButtonClass: "btn-danger",
		  confirmButtonText: 'SIM',
						...
						if (inputValue === "") {
							...
							$.post(URL_SISTEMA+"/api/financeiro", {'acao': 'liquidarDespesa', 'cod':ID, 'senha': inputValue}, function(result){
								...
								if(retorno.resposta == 1) {
									...
									swal("LIQUIDADO", "Receita liquidada com sucesso!", "success");
								} else if(retorno.resposta == 2) {
									swal.showInputError("Senha inválida");
								} else {
									swal("OOPS!", "Ocorrreu um erro inesperado, tente novamente em alguns minutos.", "error");
