...
function remover(ID){
	swal({
	  title: "Aviso",
	  text: "Você tem certeza que deseja excluír este associado?",
	  type: "warning",
	...
	function(isConfirm) {
			if(isConfirm) {
				...
				swal({
					title: "ATENÇÃO",
					text: "Por favor, digite sua senha de acesso para confirmar a exclusão do estabelecimento",
					type: "input",
					inputType: 'password',
					showCancelButton: true,
					closeOnConfirm: false,
				...
				function(inputValue){
					...
					if (inputValue === "") {
						...
						$.post(URL_SISTEMA+"/api/associados", {'acao': 'removerAssociado', 'cod':ID, 'senha': inputValue}, function(result){
							var retorno = JSON.parse(result);
							if(retorno.resposta == 1) {
								var table = $('#listar').DataTable();
								table.row( "#Iten-"+ID ).remove().draw(true);
								
								swal("REMOVIDO", "Associado removido com sucesso!", "success");
