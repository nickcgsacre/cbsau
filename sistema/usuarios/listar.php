...
define("PAGINA", "LISTAR FUNCIONÁRIOS");
define("CSS", '
...
define("JS", '
  ...
  <script type="text/javascript" src="'.URL_SISTEMA.'/sistema/usuarios/listar.js"></script>
  <!-- END PAGE VENDOR JS-->');
    ...
    <div class="content-wrapper">
      <div class="content-header row">
        <div class="content-header-left col-md-6 col-12 mb-2">
          <h3 class="content-header-title">FUNCIONÁRIOS <?=strtoupper($OPTS['plural'])?></h3>
          <div class="row breadcrumbs-top">
            <div class="breadcrumb-wrapper col-12">
              <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?=URL_SISTEMA?>">Dashboard</a>
                </li>
                <li class="breadcrumb-item active">Funcionários <?=ucfirst($OPTS['plural'])?>
                </li>
              </ol>
            </div>
          </div>
          ...
          <div class="media width-500 float-right">
            <div class="media-body media-right text-right">
			  ...
			  <div class="btn-group" role="group" aria-label="First Group">
				  ...
				  <a href="<?=URL_SISTEMA?>/usuarios/novo&tipo=<?=$_GET['tipo']?>&key=true" class="btn btn-icon btn-outline-primary"><i class="la la-plus"></i> NOVO FUNCIONÁRIO</a>
				</div>
					...
					<table class="table table-striped table-bordered sourced-data" id="listar">
                      <thead>
                        <tr>
                          ...
                          <th>Nome</th>
                          <th>Login</th>
                          <th><?=($OPTS['tipo'] == 1)? 'Cargo' : 'Estabelecimento'?></th>
                          <th>Celular</th>
                          <th>Ações</th>
                        </tr>
                      </thead>
					  <tbody>	
						...
						$LISTAR = listar("usuarios", "tipo='".$OPTS['tipo']."'", "nome ASC");
						foreach($LISTAR as $ITEM) {
						?>
                        <tr id="Iten-<?=$ITEM->id?>">
                          ...
                          <td><?=$ITEM->nome?></td>
                          <td><?=$ITEM->login?></td>
                          <td><?php if($ITEM->cargo != '0' AND $ITEM->cargo != '') { echo $CARGOS[$ITEM->cargo]->cargo; } else { echo 'N/A'; }?></td>
                          <td><?=$ITEM->celular?></td>
                          <td>
							...
							<a href="<?=URL_SISTEMA?>/usuarios/editar&id=<?=$ITEM->id?>&tipo=<?=$_GET['tipo']?>&key=true" class="btn btn-outline-info mr-1 float-right ladda-button" data-style="zoom-in">
