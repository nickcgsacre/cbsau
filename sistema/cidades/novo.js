...
function gravarForm(id, tipo) {
	...
	if(dados_Array[2]['value'] == '') {
		swal("ATENÇÃO", "Você precisa informar o nome da cidade", "info")
		return false;
	} else if(dados_Array[4]['value'] == '') {
		swal("ATENÇÃO", "Você precisa selecionar o estado", "info")
		return false;
	}
		...
		success: function( data ){
			...
			switch(dados['resposta']) {
				...
				case 1:
					...
					if(tipo == 'novo') {
						window.location.replace(`${URL_SISTEMA}/cidades/novo`);
					} else if(tipo == 'concluir') {
						window.location.replace(`${URL_SISTEMA}/cidades/listar`);
					}
