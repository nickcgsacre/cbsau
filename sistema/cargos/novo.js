...
function gravarForm(id, tipo) {
	...
	if(dados_Array[2]['value'] == '') {
		swal("ATENÇÃO", "Você precisa informar o cargo", "info")
		return false;
	}
		...
		success: function( data ){
			...
			switch(dados['resposta']) {
				...
				case 1:
					...
					if(tipo == 'novo') {
						window.location.replace(`${URL_SISTEMA}/cargos/novo`);
					} else if(tipo == 'concluir') {
						window.location.replace(`${URL_SISTEMA}/cargos/listar`);
					}
