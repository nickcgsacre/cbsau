...
if(URL_SISTEMA AND $_SESSION['user']) {
	...
	else if($_POST['acao'] == 'buscarAssociadoDependenteID') {
		$id = addslashes($_POST['termo']);
		$id = strtr($id, array (' ' => '%'));
		$RES = [];
		
		$DADOS = listar("associados", "id_associado = '".$id."'");
		foreach($DADOS as $DADO) {
			$RES[] = $DADO;
		}
		$DADOS = listar("dependentes", "id_associado = '".$id."'");
		foreach($DADOS as $DADO) {
			$RES[] = $DADO;
		}

		$Json = array("dados" => $RES);
	}
	else if($_POST['acao'] == 'buscarAssociadoDependente') {
		$termo = addslashes($_POST['termo']);
		$termo = strtr($termo, array (' ' => '%'));
		$RES = [];
		$DADOS = listar("associados", "nome like'%".$termo."%' or cpf like '%".$termo."%'");
		foreach($DADOS as $DADO) {
			$RES[] = $DADO;
		}
