...
function remover(ID){
	...
	swal({
	  title: "Aviso",
	  text: "Você tem certeza que deseja excluír este parentesco?",
	  type: "warning",
			...
			if(isConfirm) {
				...
				function(inputValue){
					...
					if (inputValue === "") {
						...
						$.post(URL_SISTEMA+"/api/parentesco", {'acao': 'remover', 'cod':ID, 'senha': inputValue}, function(result){
							var retorno = JSON.parse(result);
							if(retorno.resposta == 1) {
								var table = $('#listar').DataTable();
								table.row( "#Iten-"+ID ).remove().draw(true);
								
								swal("REMOVIDO", "Parentesco removida com sucesso!", "success");
...
function removerEmMassa(){
	swal({
	  title: "Aviso",
	  text: "Você tem certeza que deseja excluír estes parentesco?",
	  type: "warning",
	  showCancelButton: true,
	  confirmButtonClass: "btn-danger",
	  confirmButtonText: 'SIM',
			...
			if(isConfirm) {
				...
				function(inputValue){
					...
					if (inputValue === "") {
						...
						$.post(URL_SISTEMA+"/api/parentesco", {'acao': 'removerEmMassa', 'ids':camposMarcados, 'senha': inputValue}, function(result){
							...
							if(retorno.resposta == 1) {
								...
								swal("REMOVIDOS", "Parentescos removidos com sucesso!", "success");
							} else if(retorno.resposta == 2) {
								swal.showInputError("Senha inválida");
							} else {
								swal("OOPS!", "Ocorrreu um erro inesperado, tente novamente em alguns minutos.", "error");
