...
function gravarForm(id, tipo) {
	...
	if(dados_Array[2]['value'] == '') {
		swal("ATENÇÃO", "Você precisa informar o parentesco", "info")
		return false;
	}
	...
	$.ajax({
		type: "POST",
		url: URL_SISTEMA+"/api/parentesco",
		data: dados,
		success: function( data ){
			...
			switch(dados['resposta']) {
				...
				case 1:
					...
					if(tipo == 'novo') {
						window.location.replace(`${URL_SISTEMA}/parentesco/novo`);
					} else if(tipo == 'concluir') {
						window.location.replace(`${URL_SISTEMA}/parentesco/listar`);
					}
