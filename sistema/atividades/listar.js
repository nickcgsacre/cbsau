...
function remover(ID){
	...
	swal({
	  title: "Aviso",
	  text: "Você tem certeza que deseja excluír esta atividade?",
	  type: "warning",
	...
	function(isConfirm) {
			if(isConfirm) {
				...
				swal({
					title: "ATENÇÃO",
					text: "Por favor, digite sua senha de acesso para confirmar a exclusão da atividade",
					type: "input",
					inputType: 'password',
					showCancelButton: true,
					closeOnConfirm: false,
				...
				function(inputValue){
					...
					if (inputValue === "") {
						...
						$.post(URL_SISTEMA+"/api/atividades", {'acao': 'remover', 'cod':ID, 'senha': inputValue}, function(result){
							var retorno = JSON.parse(result);
							if(retorno.resposta == 1) {
								var table = $('#listar').DataTable();
								table.row( "#Iten-"+ID ).remove().draw(true);
								
								swal("REMOVIDO", "Atividade removida com sucesso!", "success");
...
function removerEmMassa(){
	swal({
	  title: "Aviso",
	  text: "Você tem certeza que deseja excluír estas atividades?",
	  type: "warning",
	  showCancelButton: true,
	  confirmButtonClass: "btn-danger",
	  confirmButtonText: 'SIM',
	...
	function(isConfirm) {
			if(isConfirm) {
				...
				swal({
					title: "ATENÇÃO",
					text: "Por favor, digite sua senha de acesso para confirmar a exclusão das atividades",
					type: "input",
					inputType: 'password',
					showCancelButton: true,
					closeOnConfirm: false,
				...
				function(inputValue){
					...
					if (inputValue === "") {
						...
						$.post(URL_SISTEMA+"/api/atividades", {'acao': 'removerEmMassa', 'ids':camposMarcados, 'senha': inputValue}, function(result){
							...
							if(retorno.resposta == 1) {
								...
								swal("REMOVIDOS", "Usuários removidos com sucesso!", "success");
							} else if(retorno.resposta == 2) {
								swal.showInputError("Senha inválida");
							} else {
								swal("OOPS!", "Ocorrreu um erro inesperado, tente novamente em alguns minutos.", "error");
